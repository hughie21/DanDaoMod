...
public class BambooSieveRecipeBuilder implements RecipeBuilder {
    ...
    private BambooSieveRecipeBuilder(RecipeCategory category, ItemLike result, int count) {
        this.category = category;
        this.result = result.asItem();
        this.count = count;
    }

    public static BambooSieveRecipeBuilder bambooSieve(RecipeCategory category, ItemLike result, int count) {
        return new BambooSieveRecipeBuilder(category, result, count);
    }

    public BambooSieveRecipeBuilder requires(ItemLike item) {
        return this.requires(Ingredient.of(item));
    }

    public BambooSieveRecipeBuilder requires(Ingredient ingredient) {
        this.ingredients.add(ingredient);
        return this;
    }

    public BambooSieveRecipeBuilder medicinalProperty(MedicinalProperties medProp, int level) {
        if (level < medProp.getMinLevel() || level > medProp.getMaxLevel()) {
            throw new IllegalArgumentException("medicinal property level is out of range");
        }

        JsonObject prop = new JsonObject();
        prop.addProperty("level", level);
    ...
    @Override
    public BambooSieveRecipeBuilder unlockedBy(String criterionName, CriterionTriggerInstance criterion) {
    ...
    public void save(Consumer<FinishedRecipe> consumer) {
        ...
        String recipePath = defaultId.getPath() + "_from_bamboo_sieve";
    ...
    public void save(Consumer<FinishedRecipe> consumer, ResourceLocation id) {
        ...
        consumer.accept(new FinishedRecipe() {
            ...
            public void serializeRecipeData(JsonObject json) {
                // 写入配方类型
                json.addProperty("type", "dandao:bamboo_sieve_recipe");
